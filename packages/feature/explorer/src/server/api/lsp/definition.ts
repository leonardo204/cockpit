/**
 * /api/lsp/definition — P8+ migration
 */
...
export const POST = handler((req) =>
  Effect.gen(function* () {
    ...
    if (!language) return ok({ definitions: [] })
    ...
    if (!server) return ok({ definitions: [] })
    ...
    const definitions = yield* Effect.tryPromise({
      try: () => server.adapter.definition(absPath, line, column),
      catch: (cause) =>
        new AppError({ message: "LSP definition failed", cause }),
    })
