...
/**
 * The in-app markdown viewer: a read-only rendering of one markdown file from
 * the project working tree, with its outline, its images, its relative-link
 * navigation and its status line.
 *
 * IT IS THE VIEWER, NOT A WINDOW. Everything about WHERE it appears — the
 * full-screen overlay, the Escape key, the close button — belongs to whoever
 * hosts it. There are two hosts and there must never be two viewers: a document
 * you glance at wants a modal (MarkdownPreviewModal) and one you keep referring
 * to wants a tab (TabManager renders this directly), but the reading experience
 * is one thing. A second copy of this pipeline is exactly the duplication
 * CLAUDE.md warns about, and it would drift the moment one host gained a fix —
 * the images, the TOC seam and the link containment are subtle enough that the
 * copy would be the broken one.
 *
 * IT IS ALSO AN ASSEMBLY, NOT A RENDERER. Every rendering decision already
 * existed in @cockpit/shared-ui and is reused verbatim — `MarkdownRenderer`
 * (GFM, math, frontmatter, alerts, Prism code blocks), `TocSidebar` (outline,
...
export function MarkdownDocument({
  ...
  cwd: string;
  /** The document to open first. Hosts key the element on this, so a second
   *  view of a different file starts from a clean history. */
  rel: string;
  /**
   * Trailing controls for the header row — the host's chrome (close, promote),
   * rendered inside the viewer's own bar so there is only one header.
   *
   * A RENDER PROP, TAKING THE DOCUMENT ACTUALLY ON SCREEN. The viewer navigates
   * between files through relative links, so `rel` is only where the reader
   * STARTED. "Open this as a tab" has to promote the document they are looking
   * at now, and lifting `current` into every host would be the same state kept
   * in two places — the version that goes stale is the one the button reads.
   */
  renderActions?: (current: string) => ReactNode;
}) {
   ...
   *
   * `fromRel` is `current`, not the `rel` the viewer was opened with: the viewer
  ...
  // O(document) work, so never inline in JSX — this re-renders on every
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-card">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-border flex-shrink-0">
        ...
        {/* The FULL project-relative path, in both hosts. A tab wears only the
            file name (there is no room for more), so this line is where the
            reader finds out which of three `README.md`s they are in. */}
